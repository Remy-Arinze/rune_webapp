  ...
  return (
    <Link href={`/tournaments/${id}`}>
      <div className="transition-transform hover:scale-105">
        <div className={cn(
          "relative w-[150px] h-[180px] rounded-lg overflow-hidden flex-shrink-0 cursor-pointer flex items-center justify-center",
          className
        )}>
          {/* Background Image */}
          <div className="absolute inset-0">
            ...
            {/* NEW: Bottom Half Blurred Overlay */}
            <div className="absolute bottom-0 left-0 right-0 h-[60%] bg-black/10 backdrop-blur-sm" />
          </div>

          {/* Content */}
          <div className="relative h-full p-4 flex flex-col justify-center items-center">
            {/* Top Section */}
            <p className="absolute top-1 right-1 text-[10px] font-bold text-[var(--primary)]">
              10,0000 RUNE
            </p>
            
            {/* Bottom Section */}
            <div className="">
              <h3 className="text-[1rem] text-center font-bold text-white">{title}</h3>
